...
fn render_bbc0_phy(radio: &Radio, area: Rect, buf: &mut Buffer) {
    ...
    let oqpsk_phr =
        Layout::horizontal([Constraint::Length(22), Constraint::Min(22)]).split(rows[11]);
...
fn render_bbc0_mac(radio: &Radio, area: Rect, buf: &mut Buffer) {
    ...
    let amcs = Layout::horizontal([Constraint::Min(60)]).split(rows[8]);
    ...
    let pmu2 = Layout::horizontal([Constraint::Length(16), Constraint::Min(16)]).split(rows[13]);
