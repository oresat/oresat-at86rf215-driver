...
//! elapsed to get a percentage. Used by the live TUI to back up the PDF's
//! "less CPU-hungry than the ax5043 reference" goal with an on-screen meter.
//!
...
/// of one CPU (e.g. `150.0` on a multi-core machine means the process is
...
/// second field (comm) is `(name)` and may contain spaces. We parse past the
...
fn read_self_ticks() -> io::Result<u64> {
    ...
    let close = raw.rfind(')').ok_or_else(|| {
        io::Error::new(io::ErrorKind::InvalidData, "no ')' in /proc/self/stat")
    })?;
