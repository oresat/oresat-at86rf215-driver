    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::NotRepresentable => {
                write!(f, "frequency/spacing not representable on 25 kHz grid")
            }
...
mod tests {
    ...
    fn freq_error_display() {
        assert_eq!(
            FreqError::OutOfRange.to_string(),
            "frequency out of supported range"
        );
        assert_eq!(
            FreqError::NotRepresentable.to_string(),
            "frequency/spacing not representable on 25 kHz grid"
        );
        assert_eq!(
            FreqError::ChannelTooLarge.to_string(),
            "channel number exceeds 9-bit field"
        );
    }
}
