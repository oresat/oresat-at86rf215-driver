...
use crate::radio::{BBC0_FBRXS, BBC0_FBTXS, BBC1_FBRXS, BBC1_FBTXS, Radio};
use crate::registers::{
    BulkWrites, ChipResetCmd, DevicePartNumber, Readable, TransceiverState, Writable,
...
pub fn open(path: &str) -> io::Result<spidev::Spidev> {
    open_with_speed(path, DEFAULT_SPI_HZ)
}

/// Open a spidev device at the given clock rate (Hz).
pub fn open_with_speed(path: &str, hz: u32) -> io::Result<spidev::Spidev> {
...
/// Read `len` bytes back from the TX frame buffer (diagnostic). The TX frame
/// buffer is readable; this confirms a prior [`write_tx_fifo`] actually landed.
pub fn read_tx_fifo(spi: &mut spidev::Spidev, bbc: Bbc, len: usize) -> io::Result<Vec<u8>> {
    let header = generate_read_header(bbc.tx_base());
    let mut cmd = Vec::with_capacity(2 + len);
    cmd.extend_from_slice(&header);
    cmd.resize(2 + len, 0x00);
    let mut rx = vec![0u8; cmd.len()];
...
pub fn wait_rf24_txprep_locked(
    ...
    loop {
        ...
        if radio.rf24_state.value.state() == TransceiverState::TxPrep && radio.rf24_pll.value.ls() {
            return Ok(());
        }
        if Instant::now() >= deadline {
            return Err(io::Error::new(
...
mod tests {
    use super::*;
    use crate::radio::{BBC0_FBRXS, BBC0_FBTXS, BBC1_FBRXS, BBC1_FBTXS};

    #[test]
    fn tx_fifo_write_command_bbc0_header() {
        let cmd = tx_fifo_write_command(Bbc::Bbc0, &[0xAA, 0xBB]);
        // Write bit (bit 15) set + BBC0 TX buffer address 0x2800.
    ...
    fn fifo_addresses_are_in_correct_ranges() {
        // Datasheet register map: BBC0 RX at 0x2000, TX at 0x2800
        //                         BBC1 RX at 0x3000, TX at 0x3800
        assert_eq!(BBC0_FBRXS, 0x2000);
        assert_eq!(BBC0_FBTXS, 0x2800);
        assert_eq!(BBC1_FBRXS, 0x3000);
        assert_eq!(BBC1_FBTXS, 0x3800);
    }
