...
/// Highlight a boolean flag: set bits bold, cleared bits dim.
...
impl Widget for RfnCn {
    fn render(self, area: Rect, buf: &mut Buffer) {
        single_cell("RFn_CN", format!("cn={} cm={}", self.cn(), self.cm())).render(area, buf);
    }
}
...
impl Widget for RfnAgcs {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        single_cell("RFn_AGCS", format!("gcw={} tgt={}dB", self.gcw(), tgt_dbfs)).render(area, buf);
    }
}
...
impl Widget for BbcnPmuval {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        single_cell("BBCn_PMUVAL", format!("{} ({:.1} deg)", raw, deg)).render(area, buf);
    }
}
...
impl Widget for BbcnAmaackt {
    fn render(self, area: Rect, buf: &mut Buffer) {
        single_cell("BBCn_AMAACKT", format!("{} microseconds", self.amaackt())).render(area, buf);
    }
}
