...
/// 0 to 360 deg in 256 steps, so each LSB is about 1.406 deg.
pub fn pmuval_degrees(pmuval: u8) -> f32 {
    pmuval as f32 * 360.0 / 256.0
}

/// Degrees -> PMUVAL. Wraps into `[0, 360)` and rounds to the nearest
/// 1.406 deg step. The PDF explicitly notes this direction has rounding
...
pub fn txcutc_khz(lpfcut: u8) -> Option<u16> {
    const TABLE: [u16; 12] = [80, 100, 125, 160, 200, 250, 315, 400, 500, 625, 800, 1000];
...
///
/// `dtb` (Duration Time Basis) selects 2/8/32/128 microseconds. `df` (Duration Factor)
/// scales it. Total = `dtb * df` microseconds. With `df = 0` the chip disables ED
...
mod tests {
    ...
    fn pmuval_from_degrees_wraps_negative_and_overrange() {
        // 360 deg wraps to 0; -90 deg wraps to 270 deg.
    ...
    fn edd_us_examples() {
        ...
        // dtb=0 -> 2 microseconds, df=10 -> 20 microseconds total.
        assert_eq!(edd_us(0, 10), 20);
        // dtb=3 -> 128 microseconds, df=4 -> 512 microseconds total.
